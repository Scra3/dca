...
def test_amount_to_spend__returns_initialise_price_and_step_price_sum():
    amount = algo.Dca(price_initialisation=20, step_price=5).compute_amount_to_spend(8, [10])
    assert amount == 25
...
def test_amount_to_spend__returns_amount_when_price_only_decrease():
    amount = algo.Dca(price_initialisation=20, step_price=1).compute_amount_to_spend(5, [10, 7])
    assert amount == 20 + 1 + 1


def test_amount_to_spend__returns_0_if_price_is_higher_that_last_price():
    amount = algo.Dca(price_initialisation=20, step_price=1).compute_amount_to_spend(13, [10, 11])
    assert amount == 0


def test_amount_to_spend__returns_0_if_price_is_higher_that_last_min_price():
    amount = algo.Dca(price_initialisation=20, step_price=1).compute_amount_to_spend(11, [10, 8, 13, 12])
    assert amount == 0
...
def test_amount_to_spend__returns_additional_amount_when_there_is_new_min_price():
    amount = algo.Dca(price_initialisation=20, step_price=1).compute_amount_to_spend(7, [10, 8, 13, 12])
