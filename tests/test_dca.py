...
def test_amount_to_spend__returns_initialise_price_when_there_is_only_one_price():
    amount = model.Dca(
        model.DcaConfiguration(
            price_initialisation=20, step_price=1, traded_pair=model.PairMapping.ETHEUR
        )
    ).compute_amount_to_spend(10, [])
    assert amount == 20
...
def test_amount_to_spend__returns_initialise_price_and_step_price_sum():
    amount = model.Dca(
        model.DcaConfiguration(
            price_initialisation=20, step_price=5, traded_pair=model.PairMapping.ETHEUR
        )
    ).compute_amount_to_spend(8, [10])
    assert amount == 20 + 5
...
def test_amount_to_spend__returns_an_amount_when_it_the_same_price_that_the_min():
    ...
    ).compute_amount_to_spend(8, [8, 8, 8])
...
def test_amount_to_spend__returns_amount_when_price_only_decrease():
    ...
    ).compute_amount_to_spend(5, [10, 7])
...
def test_amount_to_spend__returns_0_if_price_is_higher_that_last_price():
    ...
    ).compute_amount_to_spend(13, [10, 11])
    assert amount == 0
...
def test_amount_to_spend__returns_0_if_price_is_higher_that_last_min_price():
    ...
    ).compute_amount_to_spend(11, [10, 8, 13, 12])
    assert amount == 0
...
def test_amount_to_spend__returns_remaining_when_max_total_amount_to_spend_will_reached():
    amount = model.Dca(
        model.DcaConfiguration(
            ...
            step_price=1,
            max_total_amount_to_spend=11,
            traded_pair=model.PairMapping.ETHEUR,
        )
    ).compute_amount_to_spend(8, [10], total_spent=10)
    assert amount == 1
...
def test_amount_to_spend__returns_0_when_total_spent_is_bigger_than_the_max_total_amount_to_spend():
    ...
    ).compute_amount_to_spend(8, [10, 9], total_spent=12)
    assert amount == 0
...
def test_amount_to_spend__returns_additional_amount_when_there_is_new_min_price():
    ...
    ).compute_amount_to_spend(7, [10, 8, 13, 12])
...
def test_amount_to_spend__throws_error_when_price_initialisation_is_bigger_than_max_amount_to_spend():
    with pytest.raises(Exception):
        model.Dca(
            model.DcaConfiguration(
                price_initialisation=30,
                step_price=1,
                max_amount_to_spend=20,
                traded_pair=model.PairMapping.ETHEUR,
            )
        ).compute_amount_to_spend(10, [])
...
def test_amount_to_spend__returns_max_amount_to_spend_when_amount_to_spend_is_bigger():
    ...
    ).compute_amount_to_spend(7, [10, 12])
    assert amount == 20
...
def test_amount_to_spend__max_amount_to_spend_must_not_exceeded_when_force_buy_is_set():
    ...
    ).compute_amount_to_spend(4, [10, 11, 7], total_spent=30)
    assert amount == 0
...
def test_amount_to_spend__force_to_buy_with_step_price_bigger_than_1():
    ...
    ).compute_amount_to_spend(4, [10, 11, 7])
    assert amount == 26
...
def test_dca_configuration_serialise():
    ...
    assert dca_configuration.traded_pair == model.PairMapping.ETHEUR


def test_dca_configuration_serialise_raise_error_when_a_value_is_negative():
    json = (
        '{"price_initialisation": -10, "step_price": 1, '
        '"force_buy_under_price": 20, "max_amount_to_spend": 10,'
        ' "max_total_amount_to_spend": 10, "traded_pair": "ETHEUR"}'
...
def test_dca_configuration_serialise_raise_error_when_trader_pair_does_not_exist():
    json = (
        '{"price_initialisation": 10, "step_price": 1, '
        '"force_buy_under_price": 20, "max_amount_to_spend": 10,'
        ' "max_total_amount_to_spend": 10, "traded_pair": "FOOBAR"}'
    )
