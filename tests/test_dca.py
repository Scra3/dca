...
def test_amount_to_spend__returns_initialise_price_when_there_is_only_one_price():
    amount = model.Dca(
        model.DcaConfiguration(
            price_initialisation=20, step_price=1, traded_pair=api.PairMapping.ETHEUR
        )
...
def test_amount_to_spend__returns_initialise_price_and_step_price_sum():
    amount = model.Dca(
        model.DcaConfiguration(
            price_initialisation=20, step_price=5, traded_pair=api.PairMapping.ETHEUR
        )
...
def test_amount_to_spend__returns_remaining_when_max_total_amount_to_spend_will_reached():
    amount = model.Dca(
        model.DcaConfiguration(
            ...
            traded_pair=api.PairMapping.ETHEUR,
        )
...
def test_dca_configuration_serialise():
    ...
    assert dca_configuration.traded_pair == api.PairMapping.ETHEUR
